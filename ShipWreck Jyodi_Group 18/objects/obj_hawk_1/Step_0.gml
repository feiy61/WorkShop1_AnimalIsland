...
if (instance_exists(obj_Jodie_B)) { // Check if the player exists
    var _player_distance = point_distance(x, y, obj_Jodie_B.x, obj_Jodie_B.y);
   ...
   if (_player_distance < attack_range && hawkIsBlind == false && hawkAttacked == false) 
	{
        ...
        var dir_to_player = point_direction(x, y, obj_Jodie_B.x, obj_Jodie_B.y);
    ...
    if (spr_flying) {
        if (target_tree2 != noone) {
		   ...
		   if (dir_to_tree > 90 && dir_to_tree < 270) {
            ...
            if (point_distance(x, y, target_tree2.x, target_tree2.y) < 5) {
                spr_flying = false;
                perched = true;
                sprite_index = perch_sprite; // Set to perched sprite
                x = target_tree2.x;
                y = target_tree2.y - sprite_height / 2;
            }
        }
    } else if (perched) {
